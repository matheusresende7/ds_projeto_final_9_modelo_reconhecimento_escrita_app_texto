...
def boxplots( # Função para criar listas de gráficos boxplots
    ...
    rotation = 0, # Passando a rotação dos rótulos do eixo x como parâmetro da função
):
    ...
    for i, column in enumerate(columns): # Criando a estrutura de repetição para criar os gráficos
        ...
        axs[i].set_xticks(range(len(dataframe[hue_column].unique()))) # Definindo os ticks do eixo x
        axs[i].set_xticklabels(axs[i].get_xticklabels(), rotation=rotation) # Definindo a rotação dos rótulos do eixo x
